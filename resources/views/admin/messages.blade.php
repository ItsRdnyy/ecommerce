...
@section('content')
<div class="mb-8">
    <h1 class="font-serif-display text-[36px] text-gray-900 mb-2">Contact Messages</h1>
    <p class="text-[14px] text-gray-600">View and manage messages sent from the "Get in Touch" section.</p>
</div>

@if($messages->count())
<div class="bg-white border border-[#e8e5e0]">
    <div class="px-6 py-4 border-b border-[#e8e5e0]">
        <h2 class="text-[13px] font-semibold tracking-[0.1em] uppercase text-gray-700">All Messages</h2>
    </div>
    <div>
        ...
        <div class="p-6 border-b border-[#e8e5e0] last:border-b-0">
            <div class="flex items-start justify-between gap-4">
                <div class="flex-1">
                    <div class="flex items-center gap-2 mb-2">
                        ...
                        @if($message->business)
                            <span class="px-2 py-0.5 text-[10px] font-semibold bg-blue-100 text-blue-800 rounded-full uppercase tracking-wider">
                                For: {{ $message->business->businessProfile->business_name ?? $message->business->name }}
                            </span>
                        @else
                            <span class="px-2 py-0.5 text-[10px] font-semibold bg-gray-100 text-gray-800 rounded-full uppercase tracking-wider">
                                For: General / Admin
                            </span>
                        @endif
                    </div>
            ...
            <form action="{{ route('admin.messages.status', $message) }}" method="post" class="flex gap-2 mt-4">
                @csrf
                @method('patch')
...
<div class="bg-white border border-[#e8e5e0] p-16 text-center">
    <p class="text-[13px] text-gray-600">No contact messages found.</p>
</div>
@endif
@endsection
